...
const renderStackedAreaChart = (
  ...
  <AreaChart data={data}>
    <defs>
    {dataKey.map((key, i) => (
        <linearGradient key={i.toString()} id={`color-${key}`} x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor={stopColor[i][0]} stopOpacity={1} />
          <stop offset="90%" stopColor={stopColor[i][1]} stopOpacity={0.9} />
        </linearGradient>
      ))}
    </defs>
    <XAxis
    ...
    <Box style={{ width: "100%", height: "100%", paddingRight: "10px" }}>
      ...
      <Box width="100%" minHeight={260} minWidth={310} className="ohm-chart">
        {loading || (data && data.length > 0) ? (
          <ResponsiveContainer minHeight={240} width="100%">
