...
export const changeApproval = createAsyncThunk(
  ...
  async ({ token, provider, address, networkID }: IChangeApprovalAsyncThunk, { dispatch }) => {
    ...
    let approveTx;
    let stakeAllowance = await ohmContract.allowance(address, addresses[networkID].STAKING_HELPER);
      ...
      return dispatch(
        fetchAccountSuccess({
          staking: {
            ...
            bscStake: +stakeAllowance,
            bscUnstake: +unstakeAllowance
          },
        }),
      );
    }
    ...
    try {
      if (token === "ohm") {
        ...
        approveTx = await ohmContract.approve(
          addresses[networkID].STAKING_HELPER,
    ...
    stakeAllowance = await ohmContract.allowance(address, addresses[networkID].STAKING_HELPER);
...
export const changeStake = createAsyncThunk(
  ...
  async ({ action, value, provider, address, networkID }: IActionValueAsyncThunk, { dispatch }) => {
    ...
    const stakingHelper = new ethers.Contract(addresses[networkID].STAKING_HELPER as string, StakingHelperABI, signer);
